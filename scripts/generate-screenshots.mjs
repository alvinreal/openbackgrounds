...
async function generateScreenshots() {
  ...
  const filteredBackgrounds = backgrounds.filter(bg => bg.component.length > 1)
  const skippedCount = backgrounds.length - filteredBackgrounds.length

  await mkdir(SCREENSHOT_DIR, { recursive: true })

  console.log(`Starting screenshot generation for ${filteredBackgrounds.length} backgrounds...`)
  if (skippedCount > 0) {
